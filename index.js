...
PostcssPlugin.prototype.toTree = function (
  tree,
  inputPath,
  outputPath,
  inputOptions
...
module.exports = {
  ...
  included(app) {
    ...
    const overrideBrowserslist =
      this.project.targets && this.project.targets.browsers;
  ...
  postprocessTree(type, tree) {
    ...
    if (enabled && processTrees.includes(type)) {
      tree = mergeTrees([tree, new PostcssFilter(tree, this._options.filter)], {
        overwrite: true,
      });
    }
